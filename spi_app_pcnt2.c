...
static void spiTask(void* unused) {
   ...
   while(1) {
      ...
      // Buffer Clear
      spi_app_v.spiTxBuffer[0] = 0x0;
      
      // send bytes 3rd (counter data H)
      dnErr = dn_ioctl(
         DN_SPI_DEV_ID,
         DN_IOCTL_SPI_TRANSFER,
         &spiTransfer,
      ...
      cdatah = spi_app_v.spiRxBuffer[0];
      
      // send bytes 4th (counter data L)
      dnErr = dn_ioctl(
         DN_SPI_DEV_ID,
         DN_IOCTL_SPI_TRANSFER,
         &spiTransfer,
      ...
      dnm_ucli_printf(" 66 00\r\n");
      dnm_ucli_printf("SPI received:");
      dnm_ucli_printf(" %02x",cdatah);
      dnm_ucli_printf(" %02x",spi_app_v.spiRxBuffer[0]);
      dnm_ucli_printf("\r\n");
